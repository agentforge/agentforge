...
WORKDIR /app/VITS-fast-fine-tuning
RUN pip install -r requirements.txt
# build monotonic align
WORKDIR /app/VITS-fast-fine-tuning/monotonic_align/
...
RUN mkdir raw_audio
RUN mkdir denoised_audio
RUN mkdir custom_character_voice
RUN mkdir segmented_character_voice
...
RUN mv /app/VITS-fast-fine-tuning/* /app/agent_n/core/speech/vits/
# Expose port 3000
EXPOSE 3000
