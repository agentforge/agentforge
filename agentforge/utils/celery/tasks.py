...
async def send_notification(event):
    try:
        # Replace this with your logic to get the user_id from the event
        ...
        if user and "push_subscription" in user:
            ...
            try:
                ...
                print("pywebpush error in send_notification: {}", repr(ex))
        else:
            print(f"Push subscription not found for user with ID {user_id}")
