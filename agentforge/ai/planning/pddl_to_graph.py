...
def extract_last_variable_from_string(string):
    # Regex pattern to match the ?XXX pattern
    pattern = r'\?\w+'
    
    # Split the string by spaces and comma to extract individual nodes or edges
...
def trace_graph(objetcts, graph, node, visited_nodes):
    if node in visited_nodes:
        return
    visited_nodes.add(node)
    eval_node(objetcts, graph, node)
    for predecessor in graph.predecessors(node):
        edge = (predecessor, node)
        eval_edge(objetcts, graph, edge)
        trace_graph(objetcts, graph, predecessor, visited_nodes)
...
def get_seed_queries(seed: str):

    # Path to the PDDL domain file
    domain_file = '/app/agentforge/agentforge/config/configs/planner/domains/garden/domain.pddl'
    problem_file = '/app/agentforge/agentforge/config/configs/planner/domains/garden/p01.pddl'

    # Dictionary to keep track of variable types
    parameter_types = {}
    type_klasses = ["boolean", "object", "number", "string", "array", "null"]
